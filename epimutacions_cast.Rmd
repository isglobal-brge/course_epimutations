...
(1) primero identificar CpGs individualescon valores outlier y después
...
bumphubter[@jaffe2012bump] y después comprueban si en estas DMRs hay suficientes
CpGs outliers.
...
A parte, tenemos un `GenomicRatioSet` que sólo contiene muestras de casos,
...
Si tuviesemos datos crudos, la función `epi_preprocess()` permite usar
cualquiera de los 6 métodos de prepocesamiento del paquete `minfi`. Para ello, 
debemos tener todos los ficheros `IDAT` en un mismo directorio.a función se
...
Primero usaremos el dataset `methy` (section \@ref(methy)) que contiene muestras
...
método ANOVA y la función `epimutations` (diseño caso-control) - (`epi_mvo`):

```{r}
dim(epi_mvo)
class(epi_mvo)
head(as.data.frame(epi_mvo), 12)
epi_mvo[c(10:15),c(12)] <- epi_mvo[c(10:15),c(12)]*10
as.data.frame(epi_mvo)[c(10:15),c(1,7,11,12)]

kableExtra::kable(
    epi_mvo[ 9, c(1,2, 7, 10,11,12)],
    row.names = FALSE ) %>%  column_spec(1:6, width = "4cm")
...
Si ponemos el argumento `gene_annot == TRUE`, el gráfico incluye también las
anotaciones génicas:
...
Para graficar las marcas de cromatina H3K4me3, H3K27me3 y H3K27ac, especifcamos
